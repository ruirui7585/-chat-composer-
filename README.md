...
- `index.html` 是 GitHub Pages / 静态托管默认入口；
- `prototype.html` 保留为需求原型入口；
- CSS 已拆分到 `css/prototype.css`；
- JS 已拆分到 `js/prototype.js`；
- IM Chat 和 User Profile 真实截图已复制到 `assets/images/`，上传当前需求目录后不会依赖项目外层资源；
- `index.html` 和 `prototype.html` 均支持直接打开；
